...
export async function submitLead(payload: LeadPayload): Promise<boolean> {
  try {
    ...
    if (res.ok) {
      /* Meta Lead event: success handler only, once per submission */
      try { (window as unknown as { fbq?: (a: string, b: string) => void }).fbq?.("track", "Lead"); } catch { /* pixel absent */ }
    }
    return res.ok;
  } catch {
    return false;
...
export function DFooter() {
  ...
  return (
    <footer className="tex-dark" style={{ paddingTop: 60 }}>
      ...
      <div className="wrap" style={{ display: "flex", gap: 16, padding: "24px 40px 0" }}>
        {SOCIALS.map(([label, href, path]) => (
          <a key={href} className="d-social" href={href} target="_blank" rel="noopener" aria-label={label}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={path} /></svg>
          </a>
        ))}
      </div>
